  ...
  const code = useMemo(() => {
    if (content) {
      ...
      return React.icons[iconName]
        ...
        : colog(
            `EMIcon component: icon name '${iconName}' does not exist in React.icons object. ` +
              `To use icons by 'name' prop you need to make them available globally ` +
              `by adding them to React.icons object. EMIcon component docs: https://coreui.io/react/docs/components/EMIcon \n`,
            iconName
          );
    }
  }, [change]);
