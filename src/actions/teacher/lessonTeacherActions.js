...
const getLessonsTeacher = (params, page, count, query) => async (dispatch) => {
  try {
    if (!page) page = 1;
    if (!count) count = NUM_OF_LESSONS_IN_PAGE;
    dispatch({ type: ACTIONS.GET_LESSONS_REQUEST_TEACHER });
    const { data } = await authRequest.get(
      "/api/teacher/lesson/?page=" +
        page +
        "&count=" +
        count +
        (query ? "&query=" + query : "")
    );
    dispatch({ type: ACTIONS.GET_LESSONS_SUCCESS_TEACHER, payload: data });
  } catch (error) {
    dispatch({
      type: ACTIONS.GET_LESSONS_FAIL_TEACHER,
